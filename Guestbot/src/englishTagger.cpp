...
static char* tagOps [] = 
{
	...
	"PROBABLEPARTICLE",
	"PRIORTOLESSVERB","PRIORCOMMA","HOWSTART",
};
...
{
	// convert to uppercase proper name from common singular at start if appropriate (lacks determiner or adjective and isnt mass noun) and sentence is long enough
//	if (wordCount > 5 && posValues[start] == NOUN_SINGULAR && !(originalLower[start] && originalLower[start]->properties & NOUN_MASS)) // Bear went home vs Mommy went home
//	{
...
static void PerformPosTag(unsigned int start, unsigned int end)
{
	...
	if (tokenControl & NO_VERB)
	{
		...
		{
			if (ignoreWord[i]) continue;
			if ( posValues[i] & VERB_BITS && posValues[i] & NOUN_BITS && !(posValues[startSentence] & VERB_PRESENT_PARTICIPLE))
			{
				if (SetContains(MakeMeaning(imperative),MakeMeaning(canonicalLower[i])));
				else posValues[i] &= -1 ^ VERB_BITS;	
			}
		}
	}
...
{// at is the particle
	...
	if (posValues[particle-1] & (ADJECTIVE_PARTICIPLE|NOUN_INFINITIVE|NOUN_GERUND|VERB_BITS) && verb) // easy particle double
	{
	...
	else if (posValues[particle-1] & PARTICLE && posValues[particle-2] & (ADJECTIVE_PARTICIPLE|NOUN_INFINITIVE|NOUN_GERUND|VERB_BITS) && verb2) // easy particle triple from end (triples never embed?)
	{
	...
	else if (posValues[particle+1] & PARTICLE && posValues[particle-1] & (ADJECTIVE_PARTICIPLE|NOUN_INFINITIVE|NOUN_GERUND|VERB_BITS) && verb2) // easy particle triple from middle (triples never embed? object)
	{
	...
	while (--at)
	{
		if (posValues[at] & VERB_BITS) // presumed  - but wont handle dual noun like "bank teller"
		{
			break;
		}
	}
...
static int TestTag(int &i, int control, uint64 bits,int direction,bool tracex)
{
	...
	switch(control)
	{
		...
		case PRIORCOMMA:
			{
				unsigned int x = i;
				while (--x > startSentence) if (*wordStarts[x] == ',') {answer = true; break;}
				break;
			}
		case HOWSTART:
			{
		...
		case ISQUESTION: // GLOBAL   COULD this be a question (does not insure that it is, only that it could be)
			{
				answer = false;
				if (bits & AUXQUESTION && posValues[startSentence] & AUX_VERB) answer = NO_FIELD_INCREMENT;	// MAY BE A QUESTION started by aux "did you go"
...
static void WordIdioms(bool &changed)
{
	...
	for (unsigned int i = startSentence; i <= endSentence; ++i)
	{
		...
		if (diff > 3) // 5 word idioms
		{
			...
			D = FindWord(word,0,LOWERCASE_LOOKUP);
			if (!D){;}
			else if (D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))
				{
					if ((i-4)   == endSentence) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & SIGNS_OF_NOUN_BITS) &&  !(posValues[i+2] & SIGNS_OF_NOUN_BITS) ) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && posValues[i+1] & SIGNS_OF_NOUN_BITS) posValues[i] = D->properties & (PREPOSITION|CONJUNCTION);
					else posValues[i] = D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE);
				}
				else if (!(posValues[i+1] & (TO_INFINITIVE|NOUN_BITS))) posValues[i] = ADVERB;	// "a lot to process" 
				else if (posValues[i+1] & ADJECTIVE_BITS && posValues[i+2] & (ADJECTIVE_BITS|NOUN_BITS)) posValues[i] = ADVERB;	// could be noun OR adjective, we'll guess it is adjecti
				else continue;
		...
		if (!tagged && diff > 2) // 4 word idioms
		{
			...
			else if (D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))	
				{
					if ((i-3)   == endSentence) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) &&  !(posValues[i+2] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) ) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) posValues[i] = D->properties & (PREPOSITION|CONJUNCTION);
					else posValues[i] = D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE);
				}
				else if (!(posValues[i+1] & (TO_INFINITIVE|NOUN_BITS))) posValues[i] = ADVERB;	// "a lot to process" 
				else if (posValues[i+1] & ADJECTIVE_BITS && posValues[i+2] & (ADJECTIVE_BITS|NOUN_BITS)) posValues[i] = ADVERB;	// could be noun OR adjective, we'll guess it is adjecti
		...
		if (!tagged && diff > 1) // 3 word idioms
		{
			...
			else if (D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				...
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))	
				{
					if ((i-2)   == endSentence) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) &&  !(posValues[i+2] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) ) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) posValues[i] = D->properties & (PREPOSITION|CONJUNCTION);
					else posValues[i] = D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE);
				}
		...
		if (!tagged && diff > 0) // 2 word idioms
		{
			...
			else if (D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))	
				{
					if ((i-1)  == endSentence) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) &&  !(posValues[i+2] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) ) posValues[i] = ADVERB;
					else if (D->properties & CONJUNCTION && posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) posValues[i] = D->properties & (PREPOSITION|CONJUNCTION);
					else posValues[i] = D->properties & (PREPOSITION|ADVERB|CONJUNCTION_SUBORDINATE);
				}
...
static void ParseFlags0(char* buffer, unsigned int i)
{
	...
	if (parseFlags[i] & CAUSAL_DIRECTINFINITIVE_VERB) strcat(buffer,"CAUSAL_DIRECTINFINITIVE_VERB ");
	...
	if (parseFlags[i] & ADJECTIVE_NOT_SUBJECT_COMPLEMENT) strcat(buffer,"ADJECTIVE_NOT_SUBJECT_COMPLEMENT ");
...
static void Tags(char* buffer, unsigned int i)
{
	*buffer = 0;
	...
	if (type & (ADVERB))
	{
		...
		if (!(type & ADVERB)) Showit(buffer,"Adverb_unknown ",tie&ADVERB);
	}
...
void MarkTags(unsigned int i)
{
	...
	for (int j = 63; j >= 0; --j)
	{
		if (bits & bit) 
		{
			...
			else if (bit & AUX_VERB)  MarkFacts(MakeMeaning(Dauxverb),i,i); // general as opposed to specific
...
void MarkRoles(unsigned int i)
{
	...
	if (phrase && phrase != phrases[i-1])
	{
		...
		else
		{
			...
			else if (posValues[i] & (ADVERB|ADJECTIVE_BITS)) MarkFacts(MtimePhrase,i,i);
...
static bool FinishSentenceAdjust(bool resolved,bool & changed,unsigned int start,unsigned int end)
{
	...
	for (unsigned int i = start; i <= end; ++i)
	{
		...
		if (posValues[i] & ADJECTIVE_BITS && posValues[i] & ADVERB)
		{
			...
			{
				if (canSysFlags[i] & ADVERB) LimitValues(i,ADVERB,"adj/adv pending resolved by being probable adverb given no main verb exists",changed); // " Good!"
	...
	if (subjectStack[MAINLEVEL] && objectRef[verbStack[MAINLEVEL]] && verbStack[MAINLEVEL] && (!stricmp(wordStarts[subjectStack[MAINLEVEL]],"here") || !stricmp(wordStarts[subjectStack[MAINLEVEL]],"there"))) // HERE and THERE can not be the subject of a sentence:  here is the pig.. BUT  here is nice is legal.
	{
		...
		else LimitValues(subject,ADVERB,"Tere as subject is adverb not existential",changed); 
	...
	for (unsigned int i = start; i <= end; ++i)
	{
		...
		if (posValues[i] & ADVERB)
		{
			...
			else if (posValues[i+1] & (ADJECTIVE_BITS|ADVERB)) crossReference[i] = (unsigned char)(i + 1); // if next is ADJECTIVE/ADVERB, then it binds to that?  "I like *very big olives"
			else if (posValues[i+1] & VERB_BITS) //  next to next verb bind forward ("if this is tea please bring me sugar")
			{
			...
			else if (posValues[i-1] & PREPOSITION && !stricmp(wordStarts[i-1], "from") && !(posValues[i+1] & (ADVERB|ADJECTIVE_BITS|DETERMINER_BITS|NOUN_BITS|PRONOUN_BITS)))
			{
				crossReference[i] = (unsigned char)(i + 1);
			}
		...
		if (phrases[i] != phrases[i-1]  && phrases[i]) // start of a prep phrase, always needs hooking
		{
			...
			if (!(posValues[i] & PREPOSITION)) // is EITHER wrapped object from prep at end of clause or sentence, OR  its an ommited prep like omitted time prep
			{
				...
				else if (i == 1 && roles[i] & OMITTED_TIME_PREP) // find first noun or verb to link onto
				{
					...
					while (!(posValues[j] & VERB_BITS)) ++j;	 // find noun or verb
					v = j; // post verb
				}
			...
			else
			{
				if (i == 1) // find first noun or verb to link onto
				{
					unsigned int j = i;
					while (++j < endSentence && phrases[j] == phrases[j-1]){;}
					while (!(posValues[j] & (NOUN_BITS|PRONOUN_BITS|VERB_BITS))) ++j;	 // find noun or verb
...
static int ConjoinAdjective(unsigned int i, bool &changed)
{
	...
	int deepAfter = after;
	while (posValues[deepAfter] & ADVERB && !(posValues[deepAfter] & ADJECTIVE_BITS)) ++deepAfter;	// "I like green *and very red ham" can skip adverbs intervening
...
static int ConjoinNoun(unsigned int i, bool &changed)
{
	...
	if (posValues[before] == NOUN_INFINITIVE)
	{
		...
		{
			if (posValues[deepAfter] & (NOUN_BITS | VERB_BITS | AUX_VERB)) break;
		}
	...
	{
		int deepAfter = after;
		while (posValues[deepAfter] & (DETERMINER|ADVERB|ADJECTIVE_BITS|POSSESSIVE_BITS)) ++deepAfter;
	...
	int deepAfter = after;
	while (posValues[deepAfter] & (DETERMINER|ADVERB|ADJECTIVE_BITS|POSSESSIVE_BITS)) 
	{
...
static int ConjoinVerb(unsigned int i, bool &changed)
{
	...
	while (++firstAfter <= endSentence && !(posValues[firstAfter] & (NOUN_INFINITIVE|VERB_BITS))){;}		// forward to a potential verbal 

	int deepBefore = i;
	while (--deepBefore && !(posValues[deepBefore] & (VERB_BITS|NOUN_INFINITIVE))) {;}
	...
	if (posValues[before] & PARTICLE)  // he stood up and quickly ran  down the street
	{
		...
		while (++deepAfter <= endSentence)
		{
			if ( posValues[deepAfter] & (VERB_BITS|NOUN_INFINITIVE|NORMAL_NOUN_BITS|DETERMINER)) break;
		}
	...
	unsigned int deepAfter = i;
	while (++deepAfter <= endSentence && !(posValues[deepAfter] & VERB_BITS)) // forward to a potential verb 
	{	
	...
	if (posValues[deepAfter] & VERB_BITS && verbStack[MAINLEVEL]) // we and to a verb... expect this to be WORD
	{
		...
		unsigned int verb = i;
		while (--verb && !(posValues[verb] & VERB_BITS)); // find verb before
...
static bool CanBeStdAdjective(unsigned int i)
{
	...
	while (++i <= endSentence)
	{
		...
		if (posValues[i] & (ADVERB|ADJECTIVE_BITS)) continue; // part of sequence
		if (posValues[i] & (DETERMINER|PREDETERMINER|PREPOSITION)) break; // cannot find a noun after this
		if (posValues[i] & (AUX_VERB|VERB_BITS)) break; // since wasnt possibly something else, this ends hunt
	}
...
static bool ValidateSentence(bool &resolved)
{
	...
	if (resolved) for (i = startSentence; i <= endSentence+1; ++i) // walk off end of sentence
	{
		...
		{
			if (preposition && stricmp(wordStarts[preposition],"from") && !(posValues[i+1] & (ADVERB|ADJECTIVE_BITS|DETERMINER_BITS|NOUN_BITS|PRONOUN_BITS))) // "from under the bed" is legal" - from is an unusual prep 
			{
	...
	else if (!(tokenFlags & (QUESTIONMARK|EXCLAMATIONMARK))) // see if question or statement if we dont know
	{
		...
		for (i = startSentence; i <= endSentence; ++i) 
		{
			...
			if (posValues[i] & AUX_VERB && (i == startSentence || subjectFound)) // its a question because AUX or VERB comes before MAINSUBJECT
			{
...
static unsigned int GuessAmbiguousNoun(unsigned int i, bool &changed)
{
	...
	!(needRoles[roleIndex] & (MAINSUBJECT|SUBJECT2)))
	{
		...
		else if (posValues[i+1] & (VERB_BITS|AUX_VERB) && !(posValues[i+1] & (NOUN_BITS|ADJECTIVE_BITS|DETERMINER|POSSESSIVE_BITS)))
		{
	...
	if (posValues[i] & VERB_BITS && posValues[i] & NORMAL_NOUN_BITS && posValues[i+1] & (PRONOUN_BITS|NORMAL_NOUN_BITS) && originalLower[startSentence] && originalLower[startSentence]->properties & QWORD)
	{
	...
	if (needRoles[roleIndex] & (MAINSUBJECT|OBJECT2|MAINOBJECT|SUBJECT2) && posValues[i] & (NOUN_INFINITIVE|NOUN_GERUND))
	{
		...
		else if (posValues[i] & NOUN_SINGULAR && (i == endSentence || posValues[i+1] & (PREPOSITION|TO_INFINITIVE|CONJUNCTION|COMMA))) // Tashonda sent e-mail
		{
	...
	if (posValues[i] & (NOUN_PROPER_SINGULAR|NOUN_PROPER_PLURAL|NOUN_PLURAL|NOUN_SINGULAR) && posValues[i+1] & ((VERB_BITS - VERB_INFINITIVE)|AUX_VERB) && needRoles[roleIndex] & (SUBJECT2|MAINSUBJECT) && needRoles[roleIndex] & (MAINVERB|VERB2))
	{
		unsigned int at = i;
	...
	if (posValues[i-1] & (ADJECTIVE_BITS|DETERMINER|ADVERB) && posValues[i+1] & (NORMAL_NOUN_BITS|NOUN_NUMBER) && !(canSysFlags[i] & NOUN))
	{
...
static unsigned int GuessAmbiguousVerb(unsigned int i, bool &changed)
{
	...
	if ( i == startSentence && posValues[i+1] & COMMA  && posValues[i] & VERB_BITS && posValues[i] & ADVERB)
	{
	...
	if (posValues[i] & VERB_INFINITIVE && roleIndex == MAINLEVEL && needRoles[MAINLEVEL] == 0 && posValues[i+1] & CONJUNCTION)
	{
	...
	if (posValues[i] & VERB_INFINITIVE && posValues[i-1] & (PRONOUN_BITS|NOUN_BITS) && roleIndex == MAINLEVEL && needRoles[MAINLEVEL] == 0 && i != endSentence && !(posValues[i+1] & CONJUNCTION)
	...
	else if (needRoles[roleIndex] & (VERB2|MAINVERB) && posValues[i] & VERB_BITS &&  !(posValues[i] & AUX_VERB) && !(needRoles[roleIndex] & (MAINSUBJECT|SUBJECT2)))
	{
		...
		else if (posValues[i+1] & (VERB_BITS|AUX_VERB) && bitCounts[i+1] == 1) // "I *even walked home"
		{
		...
		else if (posValues[i+1] & VERB_BITS && canSysFlags[i+1] & VERB && canSysFlags[i] & ADVERB && posValues[i] & ADVERB) // "I *even make it home" - PROBABLE verb vs probable adverb
		{
			if (LimitValues(i,-1 ^ (VERB_PRESENT|VERB_PAST|VERB_PRESENT_3PS),"removingverb tense since we need a verb but next is a verb",changed)) return GUESS_RETRY;
		}
		...
		!(zoneData[currentZone] & ZONE_AUX))
	{
	 ...
	 if (posValues[i] & VERB_BITS && posValues[i] & AUX_VERB && posValues[i+1] & VERB_BITS) 
	 {
...
static unsigned int GuessAmbiguousParticle(unsigned int i, bool & changed)
{
	...
	if (!(posValues[i] & (VERB_BITS|NOUN_BITS)) && !(posValues[i+1] & (ADJECTIVE_BITS|DETERMINER|NOUN_BITS)))
	{
...
static unsigned int GuessAmbiguousAdjective(unsigned int i, bool &changed)
{
	...
	if (needRoles[roleIndex] & SUBJECT_COMPLEMENT) 
	{
		...
		if (posValues[i] == (ADJECTIVE_NORMAL|ADVERB)  && (posValues[i+1] & (CONJUNCTION|COMMA|PAREN) || i == endSentence))
		{
...
//		if (LimitValues(i,-1 ^ ADJECTIVE_BITS,"possible adjective is followed by a pronoun, so no.",changed)) return GUESS_RETRY; // might not be real -
	...
	if (posValues[i-1] & (DETERMINER|ADJECTIVE_BITS) && posValues[i] & ADJECTIVE_NORMAL && !(posValues[i+1] & (NOUN_BITS|ADJECTIVE_BITS|ADVERB)))
	{
	...
	if (posValues[i] == (ADJECTIVE_NORMAL|ADVERB) && roles[i-1] & MAINOBJECT && !(parseFlags[i] & ADJECTIVE_NOT_SUBJECT_COMPLEMENT))
	{
	...
	// if ambiguity is Adjective_basic Adverb_basic and wanting adjective as object, be adjective if next is junk "Samson was powerful but foolish"  but not "Max looked *all_over for her" and not "the ball was *inside"
	if (posValues[i] == (ADJECTIVE_NORMAL|ADVERB) && needRoles[roleIndex] & SUBJECT_COMPLEMENT && (posValues[i+1] & (CONJUNCTION|COMMA|PAREN) || i == endSentence) &&
...
{	
	...
	if (i != startSentence && posValues[i] & ADVERB && !stricmp(wordStarts[i], "but") && posValues[i+1] & ADJECTIVE_BITS)  // "there is but one god." dont make conjunction or prep
	{
	...
	if (needRoles[roleIndex] & SUBJECT_COMPLEMENT && posValues[i] & ADVERB && posValues[i+1] & ADJECTIVE_BITS) // "they  are *most active at night"
	{
	...
	if (i != startSentence && posValues[i] & ADVERB && posValues[i+1] & (DETERMINER_BITS|NOUN_BITS|PRONOUN_BITS) && bitCounts[i+1] == 1 ) // "i have a paper to write *before class"
	{
	...
	if (posValues[i] & ADJECTIVE_NORMAL && posValues[i] & ADVERB && posValues[i+1] == ADJECTIVE_NORMAL)
	{
	...
	if (canSysFlags[i] & ADVERB && posValues[i-1] & AUX_VERB && posValues[i+1] & VERB_BITS)
	{
	...
	if (posValues[i-1] & (VERB_BITS|NOUN_GERUND|NOUN_INFINITIVE) && posValues[i+1] & CONJUNCTION)
	{
	...
	if (posValues[i] & ADVERB && posValues[i+1] & ADVERB && originalLower[i+1] && originalLower[i+1]->systemFlags & TIMEWORD )
	{
...
static unsigned int GuessAmbiguousPreposition(unsigned int i, bool &changed) // and TO_INFINITIVE
{
	...
	if (posValues[i] & ADVERB && !stricmp(wordStarts[i], "but") && posValues[i+1] & (NOUN_BITS|ADJECTIVE_BITS|PRONOUN_BITS|DETERMINER_BITS) && posValues[i-1] & (VERB_BITS))  // "there is but one god." dont make conjunction
	{
	...
	if (posValues[i] & PARTICLE && posValues[i+1] & CONJUNCTION)
	{
	...
	// if noun evidence follows it, be not an adverb unless it is sentence start 
	if (i == startSentence && posValues[i] & ADVERB && posValues[i+1] & (DETERMINER_BITS|NOUN_BITS|PRONOUN_BITS) ) // "*next I went home"
	{
...
static unsigned int GuessAmbiguousDeterminer(unsigned int i, bool & changed) // and predeterminer
{
	...
	if (i == startSentence && posValues[i] & PRONOUN_BITS && posValues[i] & DETERMINER_BITS && posValues[i+1] & NOUN_BITS  &&  posValues[i+1] & (VERB_BITS & AUX_VERB) )
	{
	...
	{
		if (posValues[i+1] & (AUX_VERB|VERB_BITS) && needRoles[roleIndex] & (MAINVERB|VERB2)) {;} // not "*what will my dog do?"
...
static unsigned int GuessAmbiguousPronoun(unsigned int i, bool &changed)
{
	...
	if (i == startSentence && posValues[i+1] & PRONOUN_SUBJECT && posValues[i] & DETERMINER)
	{
	...
	if (posValues[i] & (ADVERB) && posValues[i+1] & (PRONOUN_BITS))
	{
	...
	if (i == startSentence && posValues[i] & PRONOUN_BITS && posValues[i+1] & AUX_VERB && bitCounts[i+1] == 1 && posValues[endSentence] == PREPOSITION)
	{
	...
	if (phrases[i-1] && !(posValues[i-1] & (NOUN_BITS|PRONOUN_BITS)) && posValues[i] & PRONOUN_BITS && posValues[i+1] & AUX_VERB && bitCounts[i+1] == 1)
	{
		if (LimitValues(i,PRONOUN_BITS,"requiring pronoun object since aux follows and prep ends sentence",changed)) return GUESS_RETRY; // might not be real
	}
...
static unsigned int GuessAmbiguousAux(unsigned int i, bool &changed)
{
	...
	if (roleIndex == MAINLEVEL && roleIndex == MAINLEVEL && !(needRoles[roleIndex] & (VERB2|MAINVERB)) && phrases[i-1])
	{
	 ...
	 if (posValues[i+1] & VERB_INFINITIVE && posValues[i+1] & (DETERMINER|ADJECTIVE_BITS|NOUN_BITS) && !(posValues[i+1] & CONJUNCTION)) // but not "*do as I say"
	 {
	 ...
	 if (posValues[i] & VERB_BITS && posValues[i] & AUX_VERB && posValues[i+1] & (DETERMINER|ADJECTIVE_BITS|NOUN_BITS) && !(posValues[i+1] & CONJUNCTION)) // but not "*do as I say"
	 {
	...
	if (i == startSentence && posValues[i] & VERB_INFINITIVE && posValues[i+1] & (PREPOSITION | CONJUNCTION))
	{
...
static unsigned int GuessAmbiguousConjunction(unsigned int i, bool &changed)
{
	...
	unsigned int at = i;
	while (++at < endSentence && !(posValues[at] & VERB_BITS)) {;}
	if (!(posValues[at] & VERB_BITS)) 
	{
	...
	if (i == startSentence && originalLower[startSentence]->properties & QWORD && posValues[i+1] & (AUX_VERB | VERB_INFINITIVE))
	{
...
// SUBJECT clause starters immediately follow a noun they describe
	...
	{
		if (posValues[i+1] & AUX_VERB && !(needRoles[roleIndex] & (MAINOBJECT|OBJECT2))) return;	// more a real sentence, UNLESS we were looking for an object "I ate what was given"
		...
		if (!clauses[i])
		{
			...
			else if (!stricmp(wordStarts[i],"which") || !stricmp(wordStarts[i],"that") ) // wh-determiners (or simple clause starters)
			{
				...
				else if (posValues[i+1] & (AUX_VERB|VERB_BITS))
				{
...
//[subject and verb omitted from adverb clause]
		...
		if (needRoles[MAINLEVEL] & MAINVERB)
		{
			...
			while (++at <= endSentence)
			{
				if (posValues[at] & VERB_BITS) ++count;
			}
		...
		if (prior && posValues[prior] & NOUN_BITS && posValues[i] & (NOUN_BITS | ADJECTIVE_NOUN | PRONOUN_BITS) && !(posValues[i] & (-1 ^ (NOUN_BITS | ADJECTIVE_NOUN| PRONOUN_BITS))) && !(needRoles[roleIndex] & (MAINSUBJECT|MAINOBJECT|OBJECT_COMPLEMENT|OBJECT2)) )
		{
			...
			if (posValues[i+1] & (AUX_VERB|VERB_BITS) && !roles[i])
			{
	...
	for (unsigned int i = startSentence; i <= endSentence; ++i)
	{
		...
		if (trace & TRACE_POS)
		{
			...
			*tags = 0;
			Tags(tags,i);
			strcat(tags,flags);
			Log(STDUSERLOG,"%d) \"%s %s\" (%s)\r\n",i,word,(canonicalLower[i]) ? canonicalLower[i]->word : canonicalUpper[i]->word,tags);
		...
		case VERB_PRESENT: case VERB_PRESENT_3PS: case VERB_PAST: case VERB_PAST_PARTICIPLE: case VERB_PRESENT_PARTICIPLE: //  VERB_BITS
			...
			case ADVERB: // adverb  binds to PRIOR verb, not next verb unless next to next verb ("if this is tea *please bring me sugar")
			{
				...
				else if (posValues[i+1] & VERB_BITS) //  next to next verb ("if this is tea please bring me sugar")
				{
					if (clauses[i+1]) clauses[i] = clauses[i+1];
					if (verbals[i+1]) verbals[i] = verbals[i+1];
					crossReference[i] = (unsigned char)(i + 1);
			...
			case COMMA: // close needs for indirect objects, ends clauses e.g.   after being reprimanded, she went home
			{
				...
				if (posValues[i-1] & ADJECTIVE_BITS && posValues[i+1] & (NOUN_BITS|ADVERB|ADJECTIVE_BITS|CONJUNCTION_COORDINATE)) break; 
