...
retry:
			...
			{
				if (originalLower[i]  && posValues[i] & ADVERB && lcSysFlags[i] & MORE_FORM) ++right;
				else Log(STDUSERLOG,"** Bad RBR at %d: line %d %s %s %s\r\n",i,currentFileLine,mytags[i],tags[i],buffer);
			}
			else if (!stricmp(tags[i],"RBS")) 
			{
				if (originalLower[i]  && posValues[i] & ADVERB && lcSysFlags[i] & MOST_FORM) ++right;
...
static bool DumpUpHierarchy(MEANING T,int depth)
{
    ...
    if (depth == 0)  
	{
		...
		{
			if (index && k != index) continue; //   not all, just correct meaning
			MEANING M = GetMeaning(E,k);
			if (restrict && !(M & restrict)) continue; // not valid meaning
